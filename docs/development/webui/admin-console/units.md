...
two sub-views, **Registered Units** and **Pending**, plus a live badge count of pending robots. This page
...
A row in `units` only means the robot is registered as a unit. Whether anyone can drive it is decided
...
**Contract:** `PATCH /admin/api/units/:id` (`unit_name`); see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).
...
nothing about routing, the unit relay's roster, or any topic the robot publishes to.

### Delete a unit

**Contract:** `DELETE /admin/api/units/:id`; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).

Removes the `units` row, gated by a confirmation that is explicitly aware the change may not have
reached the running system yet. That staleness is real, not defensive UI copy: the unit relay
...
**Contract:** `POST /admin/api/units/:id/backups` with `{ profile_id }` or `{ all_profiles: true }`; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).
...
**Contract:** `POST /admin/api/units/:id/swap` with `target_unit_id` and a rental scope; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).
...
**Contract:** `DELETE /admin/api/units/:id/data` with a rental scope; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).
...
**Contract:** `POST /admin/api/units/:id/transfer` with `target_unit_id` and `profile_id` or `all_profiles`; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).
...
**Contract:** `DELETE /admin/api/units/:id/device`; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api).
...
**Contract:** `GET /admin/api/pending-units`, `POST /admin/api/pending-units/:id/register` (`unit_name`) or `/adopt` (`unit_id`), `DELETE /admin/api/pending-units/:id`; see [HTTP API § Admin API](/development/message-contracts/http-api#admin-api). The robot's side of the handshake: [Firmware & Enrolment § Enrolment](/development/message-contracts/firmware-and-enrolment#enrolment).
...
- [Unit Container Lifecycle](/development/unit-container-lifecycle): the standalone reference for `unit_manager.js`, the roster, and the unit relay.
