...
<LinkCards>
  ...
  <LinkCard icon="📨" title="Message Contracts" details="Every HTTP, MQTT, ROS and WebSocket payload, with an action trace from each dashboard button to its messages." link="/development/message-contracts/" />
...
5. [Message Contracts](/development/message-contracts/): Trace any dashboard action to the exact HTTP, MQTT and ROS messages it sends.
