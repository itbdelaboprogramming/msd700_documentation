...
1. **10 Seconds (Motion Pause)**: If no valid heartbeat arrives for 10 seconds (`ping_pause_timeout 10.0`), `system_command.py` latches `/emergency_pause` (`std_msgs/Bool`) and `emergency_stop_node` floods zero-twist on `/mux/emergency_vel` at priority 255. The robot decelerates to a complete stop without canceling the active `move_base` goal. When communication returns, the pause is lifted and motion resumes automatically.
