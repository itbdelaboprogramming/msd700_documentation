...
`docker-manager.sh` and `run_msd.sh` (see [Docker Reference](/setup/docker-reference)) assume the invoking user can run `docker` without `sudo`. Add the user to the `docker` group and start a new shell session for it to take effect:
