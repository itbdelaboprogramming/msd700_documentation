...
**Validation** (enforced in the agent, not just the form): SSID is 1–32 **octets** — a name in a
non-Latin script hits the limit sooner than its character count suggests — and the WPA-PSK password
is 8–63 characters. Control characters are rejected rather than stripped, since quietly sanitising
