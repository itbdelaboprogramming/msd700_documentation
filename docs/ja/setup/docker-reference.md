...
| `--fresh` | `reenroll`のみ | ローカルキャッシュ(`mysql_data_local`、`media_data_local`、`mosquitto_data_local`)も消去する。**新しい**レンタルに出すロボット向け。指定しない場合キャッシュは保持され、フルシンクで整合される |
| `--no-archive` | `reenroll`のみ | バックアップを省略する。ユニットが保持データをすべて同期済みの場合のみ安全。そうでなければオフラインのマップが失われる |
| `-y`, `--yes` | `reenroll`のみ | ユニットIDの確認を含め、プロンプトを出さない |
