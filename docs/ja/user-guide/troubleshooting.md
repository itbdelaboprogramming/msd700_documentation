...
- **オペレーターが行う操作**:
  ...
  3. インターネットのないローカル施設ネットワークで運用している場合は、ロボットのWi-Fiホットスポットに接続し、`http://mymsd.jp`(別のローカルネットワーク経由の場合は `http://<robot-ip>:3000`)を開いていることを確認します。
...
- **考えられる原因**:
  ...
  - ロボットが約1.24メートル(旋回に必要な幅)より狭い通路でエリアカバレッジ清掃を試みています。0.80 mの通路には進入できますが、その中で旋回はできません。
- **オペレーターが行う操作**:
