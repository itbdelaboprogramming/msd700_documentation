...
[その場回転ガード](/ja/development/ros/boustrophedon-and-alignment#その場回転はデフォルトで拒否される)はデフォルトで無人の回転を拒否するため、プラットフォームにはその動作に頼らずポーズを補正するオペレーター向けの手段が必要になる。
...
[Boustrophedonカバレッジ & Zero-Spinアラインメントアーキテクチャ](/ja/development/ros/boustrophedon-and-alignment#ゼロスピン方位アライメント-correlative-scan-matching)で完全に文書化されている、同じゼロスピンのCorrelative
...
[その場回転ガード](/ja/development/ros/boustrophedon-and-alignment#その場回転はデフォルトで拒否される)にとって不可欠な支えとなっている。このガードは、2つの同意トピックのいずれかにライブコマンドが伴わない限りすべてのその場回転を拒否するが、Auto
...
[Boustrophedonカバレッジ & Zero-Spinアラインメントアーキテクチャ](/ja/development/ros/boustrophedon-and-alignment#その場回転はデフォルトで拒否される)で文書化されており、ここでは繰り返さない。
