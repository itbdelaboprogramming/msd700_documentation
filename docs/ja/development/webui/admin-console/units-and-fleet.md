...
で説明されている、物理ロボット1台につき1行のテーブルだ。**Fleet** と **Pending** の2つのサブビュー
...
`units` の1行は、そのロボットがフリートに存在することのみを意味する。誰かがそれを操縦できるか
...
`unit_name`)。これは、
[データベーススキーマ § エンロールメント](/ja/development/database-schema#enrolment) で説明
...
（`/unit_<id>/...`）である。ユニットの名前を変更しても、ルーティング、フリートリレーのロスター、
ロボットが発行するどのトピックについても何も変わらない。

### ユニットを削除する

`units` の行を削除する。この変更がまだ実行中のシステムに反映されていない可能性を明示的に認識
した確認によってゲートされる。その古さは実在するものであり、防御的な UI 文言ではない。フリート
...
[バックアップとリストア](/ja/development/backup-and-restore#dual-scope-backup-architecture)
...
回復できないのかについては
[ROS連携 § ユニットのエンロールメントとバインド解除](/ja/development/webui/admin-console/ros-integration#unit-enrolment-and-unbinding)
を参照。
...
[データベーススキーマ § エンロールメント](/ja/development/database-schema#enrolment))に存在し、
...
- [ユニットコンテナライフサイクル](/ja/development/unit-container-lifecycle): `unit_manager.js`、ロスター、フリートリレーの単独リファレンス。
