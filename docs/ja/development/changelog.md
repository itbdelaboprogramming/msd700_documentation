...
- **2秒の安全一時停止とMQTTハートビート**: ウォッチドッグの動作一時停止は、オペレーター不在2秒で発動するようになった(従来10秒)。ユニットのローカルダッシュボードは、5 HzのMQTT over WebSocket `heartbeat`をユニットのブローカーへ直接送って在席を証明するため、損失の多い回線でロボットが止まらなくなった。リースと権限は引き続きHTTP pingが担う。Autopilotはウォッチドッグの3階層すべてを抑制する。
- **実測の車輪形状**: `pose_config.yaml`は実測のトラック26 cmを使うようになり(従来78 cmで旋回が毎回3倍行き過ぎていた)、`rotation_guard`ノードは削除された。
- **狭い隙間向けTEBチューニング**: `min_obstacle_dist` 0.10 → 0.05 m、`inflation_dist` 0.75 → 0.35 m。`navfn`が計画できる隙間でTEBが止まらなくなった。これらから導出される網羅走行のクリアランスも小さくなった。
- **ユニットでのC++版topic2string**: ユニットのテレメトリ変換は個別のC++ノードで動作する(`topic2string_impl:=cpp_nodes`)。Pythonノードはロールバック用に残されている。
- **高速化したハザードスキャン**: 知覚パイプラインのグループ集約をCライブラリ(`fastops`)に移し、1フレームを約55 msから約37 msに短縮した。numpyへのフォールバックあり。
- **ダッシュボードからのホットスポット再起動**: `network_local`はNetworkManagerではなく、範囲を絞った`msd700-hotspot-restart.path`ユニット経由でホットスポットを再起動する。
- **自動デプロイWebhook**: デプロイ用ブランチへのpushで`server_prod` / `server_dev`が自動的に再ビルドされる。
- **draw.io による図**: ドキュメントの図はすべて、ページの隣に置いた編集可能な `.drawio` ファイルになり、draw.io ビューアーで静的な PNG にレンダリングされる。Mermaid は使用しなくなった。[リポジトリ構成 § 図](/ja/development/repository-structure#図)を参照。
