...
| `FLEET_CLIENT_ID` | `fleet_nakayama_cloud` | フリートリレー専用。1つの共有接続のための MQTT クライアント ID。`clean_session` が true であるためブローカーごとに一意である必要があり、重複する ID はもう一方のクライアントを切断し、両者がフラップし合う。 |
...
フリートリレーは、データプレーンの**両方の半分**を1つのコンテナ `rosweb_unit_relays` に統合します。
