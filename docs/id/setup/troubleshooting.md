...
Technical diagnostics for installation and deployment issues. For user-facing issues, see [User Guide &gt; Troubleshooting](/id/user-guide/troubleshooting) instead.
...
5. **Is the network path open**, on the ports in
   [System Setup](/id/setup/system-setup#_1-konfirmasi-jalur-jaringan)?
...
| Camera feed works on the LAN, never from outside | The TURN relay is advertising an unreachable address, or its ports are not forwarded | Check `TURN_EXTERNAL_IP` and the router forward; see [Maintenance](/id/setup/maintenance#relay-turn) |
